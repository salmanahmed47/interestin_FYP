...
class _SelectedEventState extends State<SelectedEvent> {
  ...
  Future<void> fetchRegisteredUsers() async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      for (QueryDocumentSnapshot<Map<String, dynamic>> document in querySnapshot.docs) {
        CollectionReference registeredUsersCollection = document.reference.collection('registeredUsers');
        ...
        if (registeredUsersSnapshot.docs.isNotEmpty) {
          List<String> registeredusers = registeredUsersSnapshot.docs.map((doc) => doc['email'] as String? ?? '').toList();
          setState(() {
  ...
  Future<void> fetchUsersInLocation() async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      for (QueryDocumentSnapshot<Map<String, dynamic>> document in querySnapshot.docs) {
        CollectionReference usersInLocationCollection = document.reference.collection('usersInLocation');
        QuerySnapshot<Map<String, dynamic>> usersInLocationSnapshot =
            await usersInLocationCollection.get() as QuerySnapshot<Map<String, dynamic>>;

        if (usersInLocationSnapshot.docs.isNotEmpty) {
          List<String> usersinlocation = usersInLocationSnapshot.docs.map((doc) => doc['email'] as String? ?? '').toList();
          setState(() {
          ...
          for (Map<String, String> user in toBlockUsers) {
            if (user['time'] == '0') {
              ...
              if (userSnapshot.docs.isNotEmpty) {
                DocumentSnapshot<Map<String, dynamic>> userDocSnapshot = userSnapshot.docs[0];
                int currentBlockCount = userDocSnapshot.data()?['blockCount'] ?? '0';
