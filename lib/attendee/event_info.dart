...
class _EventInfoState extends State<EventInfo> {
  ...
  void showRegistrationsFullDialog(BuildContext context, String title, String message) {
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
  ...
  void storeRegisteredEvents() async {
    ...
    CollectionReference eventsRef = FirebaseFirestore.instance.collection('users');
    QuerySnapshot eventsDocumentSnapshot = await eventsRef.where('email', isEqualTo: email).get();
    ...
    if (eventsDocumentSnapshot.docs.isNotEmpty) {
      ...
      CollectionReference subcollectionRef = documentRef.collection('registeredEvents');
  ...
  void getBlockCount() async {
    ...
    QuerySnapshot querySnapshot = await usersRef.where('email', isEqualTo: userEmail).get();
  ...
  Future<void> registerForEvent() async {
    ...
    QuerySnapshot querySnapshot =
        await eventsRef.where('title', isEqualTo: widget.title).where('description', isEqualTo: widget.description).get();
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      if (capacity > numOfRegisteredUsers) {
        if (blockCount < 3) {
          ...
          String documentId = documentSnapshot.id;
          DocumentReference documentRef = eventsRef.doc(documentId);
          CollectionReference subcollectionRef = documentRef.collection('registeredUsers');

          QuerySnapshot registeredUsersSnapshot = await subcollectionRef.where('email', isEqualTo: email).get();

          if (registeredUsersSnapshot.docs.isNotEmpty) {
            showAlertDialog(context, 'Already Registered', 'You have already registered for this event.');
          } else {
          ...
          showRegistrationsFullDialog(context, "Can't register", "you are blocked");
        }
      } else {
        showRegistrationsFullDialog(context, "Can't register", "Event capacity is full so you can't register for this event.");
      }
  ...
  Widget build(BuildContext context) {
    String sdate = DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.startDate));
    sdate.split(" ");

    String edate = DateFormat('dd/MM/yyyy').format(DateTime.parse(widget.endDate));
    sdate.split(" ");
          ...
          child: Column(
            ...
            children: [
              if (widget.images.isNotEmpty) ...[
                ...
                carouselSliderWidget(widget.images),
              ],
              const SizedBox(
                height: 50,
              ),
                              ...
                              child: Text.rich(
                                TextSpan(
                                  children: [
                                    ...
                                    TextSpan(text: "${widget.mobile} , ${widget.email}"),
                                  ],
                                ),
                              ),
                            ),
